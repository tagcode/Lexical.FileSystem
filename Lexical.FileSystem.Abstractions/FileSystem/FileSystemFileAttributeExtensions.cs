...
{
    /// <summary>
    /// Extension methods for setting FileAttributes on <see cref="IFileSystem"/>.
    /// 
    /// File Attributes are read with <see cref="IFileSystemBrowse.GetEntry(string, IOption)"/> and <see cref="IEntryFileAttributes"/>.
    /// </summary>
    public static partial class FileSystemFileAttributeExtensions
    {
        /// <summary>
        ...
        public static void SetFileAttribute(this IFileSystem filesystem, string path, FileAttributes fileAttribute, IOption option = null)
        {
            ...
            else if (filesystem is IFileSystemFileAttributeAsync attributerAsync) attributerAsync.SetFileAttributeAsync(path, fileAttribute, option).Wait();
        ...
        /// <exception cref="InvalidOperationException">If <paramref name="path"/> refers to a non-file device, such as "con:", "com1:", "lpt1:", etc.</exception>
        /// <exception cref="ObjectDisposedException"></exception>
        public static Task SetFileAttributeAsync(this IFileSystem filesystem, string path, FileAttributes fileAttribute, IOption option = null)
        {
            if (filesystem is IFileSystemFileAttributeAsync attributerAsync) return attributerAsync.SetFileAttributeAsync(path, fileAttribute, option);
            else if (filesystem is IFileSystemFileAttribute attributer) return Task.Run(()=>attributer.SetFileAttribute(path, fileAttribute, option));
            else throw new NotSupportedException(nameof(SetFileAttribute));
        }
