...
namespace Lexical.FileSystem
{
    ...
    public static partial class FileSystemDeleteExtensions
    {
        ...
        public static void Delete(this IFileSystem filesystem, string path, bool recurse = false, IOption option = null)
        {
            ...
            else if (filesystem is IFileSystemDeleteAsync deleterAsync) deleterAsync.DeleteAsync(path, recurse, option).Wait();
        ...
        /// <exception cref="InvalidOperationException"><paramref name="path"/> refers to non-file device</exception>
        /// <exception cref="ObjectDisposedException"/>
        public static Task DeleteAsync(this IFileSystem filesystem, string path, bool recurse = false, IOption option = null)
        {
            if (filesystem is IFileSystemDeleteAsync deleterAsync) return deleterAsync.DeleteAsync(path, recurse, option);
            else if (filesystem is IFileSystemDelete deleter) return Task.Run(()=>deleter.Delete(path, recurse, option));
            else throw new NotSupportedException(nameof(Delete));
        }
    }
